...
const GenericPage: React.FC<{ title: string; subtitle: string; icon: React.ReactNode; videoUrl?: string }> = ({ title, subtitle, icon, videoUrl }) => (
  <div className="relative min-h-screen flex items-center justify-center px-6 bg-[#001529] overflow-hidden">
    {videoUrl && (
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#001529]/80 via-transparent to-[#001529]" />
      </div>
    )}
    ...
    >
      <div className="inline-block p-10 bg-white/5 rounded-[40px] border border-white/10 backdrop-blur-3xl shadow-2xl">
        {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement<any>, { className: "w-16 h-16 text-[#219EBC] drop-shadow-[0_0_15px_#219EBC]" }) : icon}
      </div>
      <h2 className="text-6xl md:text-9xl font-serif text-white font-light uppercase tracking-widest leading-none">{title}</h2>
